...
func _ready():
	...
	if Globals.get("gj_username"):
		username_cache = Globals.get("gj_username")
		print("Preauthed, ", username_cache)
	if Globals.get("gj_token"):
		token_cache = Globals.get("gj_token")
		print("Preauthed, ", token_cache)
...
func data_store_set(key, data):
	...
	request(url + "&signature=" + signature)
	print("Requested to store data, ", data)
	pass
